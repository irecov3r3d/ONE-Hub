...
// Hoisted static frequency band configuration to eliminate per-call object allocations
...
export class ReferenceMatchingService {
  ...
  compareToReference(
    ...
    // Calculate overall similarity using precomputed differences to avoid redundant computations
   ...
   * Accepts optional precomputed TrackDifferences to eliminate duplicate calculations in compareToReference
   */
  public calculateSimilarity(
    target: AudioAnalysisResult,
    reference: AudioAnalysisResult,
    precomputedDifferences?: TrackDifferences
  ): number {
    let similarity = 100;

    // Loudness difference (30% weight)
    const lufsDiff = precomputedDifferences
      ? Math.abs(precomputedDifferences.loudness.lufsDiff)
      : Math.abs(target.loudness.integratedLUFS - reference.loudness.integratedLUFS);
    similarity -= Math.min(30, lufsDiff * 3);

    // Frequency balance (40% weight)
    const freqDiff = precomputedDifferences
      ? precomputedDifferences.frequency
      : this.compareFrequency(target, reference);
    ...
    const stereoWidthDiff = precomputedDifferences
      ? Math.abs(precomputedDifferences.stereo.widthDiff)
      : Math.abs(target.stereo.stereoWidth - reference.stereo.stereoWidth);
    similarity -= Math.min(15, stereoWidthDiff * 0.3);

    // Dynamic range (15% weight)
    const drDiff = precomputedDifferences
      ? Math.abs(precomputedDifferences.loudness.dynamicRangeDiff)
      : Math.abs(target.loudness.dynamicRange - reference.loudness.dynamicRange);
