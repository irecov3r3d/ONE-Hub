...
export class FastFFTEngine {
   ...
   * ⚡ Bolt Optimization:
   * 1. Supports polymorphic input (AudioBuffer | Float32Array) for zero-copy analysis.
   * 2. Uses .subarray() to avoid copies when processing buffer segments.
  ...
  async performFFT(
    ...
    let sampleRate = sampleRateOverride || 44100;
    let samples: Float32Array;

    if (input instanceof (typeof AudioBuffer !== 'undefined' ? AudioBuffer : Object)) {
      const buffer = input as AudioBuffer;
      sampleRate = sampleRateOverride || buffer.sampleRate;
      const channelData = buffer.getChannelData(0);

      // Use middle portion for analysis of full buffers
    ...
    const fftResult = FastFFTEngine.cooleyTukeyFFT(inputToFFT, undefined, window);
