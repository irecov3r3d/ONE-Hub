...
export class FastFFTEngine {
   ...
   * ⚡ Bolt Optimization:
   * 1. Supports direct Float32Array input for zero-copy analysis of segments.
  ...
  async performFFT(
    ...
    if (input instanceof Float32Array) {
      ...
      samples = input.length === fftSize ? input : input.subarray(0, Math.min(input.length, fftSize));
    } else {
