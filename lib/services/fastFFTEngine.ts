...
export class FastFFTEngine {
   ...
   * ⚡ Bolt Optimization:
   * 1. Polymorphic input: supports zero-copy analysis of raw samples (Float32Array).
   * 2. Removed unused Web Audio API objects for static buffer analysis.
  ...
  async performFFT(
    ...
    const isAudioBuffer = (input as any).getChannelData !== undefined;
    ...
    // If channelData is exactly fftSize, use it directly to support pre-windowed segments.
    // Otherwise, take the middle portion for a representative sample.
