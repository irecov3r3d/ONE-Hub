...
export class AudioAnalysisService {
   ...
   * ⚡ Bolt Optimization: Reuses pre-calculated 8192-point FFT magnitudes for key detection and pitch analysis.
   */
  private async analyzeMusicalFeatures(
    ...
    // ⚡ Bolt: Real Krumhansl-Schmuckler key detection with spectral reuse
    const keyData = this.keyDetector.detectKeyFromMagnitudes(
      magnitudes8192,
      audioBuffer.sampleRate,
      8192
    );

    // ⚡ Bolt: Reuse chromagram from key detection for pitch class analysis
    const pitchClasses = this.convertChromaToPitchClasses(keyData.chromagram);
  ...
  /**
   * Convert normalized chromagram to PitchClass array.
   * ⚡ Bolt Optimization: Reuses chromagram calculated during key detection.
   */
  private convertChromaToPitchClasses(chromagram: number[]): PitchClass[] {
