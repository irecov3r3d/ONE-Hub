...
export class AudioAnalysisService {
  ...
  private async analyzeFrequency(
    ...
    // ⚡ Bolt: Pre-calculate linear magnitudes into a single Float32Array.
    // This eliminates ~40,000 redundant Math.pow calls across subsequent spectral analyses.
    const linearMagnitudes = new Float32Array(numBins);
    let totalEnergy = 0;
  ...
  private analyzeFrequencyBand(
    ...
    for (let i = minBin; i <= maxBin && i < spectrum.length; i++) {
      ...
      sumEnergy += linearMagnitudes[i]; // ⚡ Bolt: Reuse pre-calculated linear magnitude
      count++;
    }
   ...
   * ⚡ Bolt: Uses pre-calculated linear magnitudes and total energy.
   */
   ...
   * ⚡ Bolt Optimization: Defer note conversion until after top 10 selection to avoid
   * thousands of expensive Math.log2/Math.pow calls.
   */
  private findDominantFrequencies(
    ...
    // Apply note conversion only to the winners
    for (const freq of top10) {
      freq.note = this.frequencyToNote(freq.frequency);
    }
