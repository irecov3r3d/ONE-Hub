...
interface BasicAudioStats {
  ...
  totalSamples: number;
  length: number;
  // ⚡ Bolt: Block-level metrics for optimized downstream analysis
...
export class AudioAnalysisService {
  ...
  async analyzeAudio(file: File): Promise<AudioAnalysisResult> {
    ...
    // ⚡ Bolt: Single-pass stats collection (includes mono conversion and block metrics)
   ...
   * Also pre-calculates block-level metrics for temporal and loudness analysis.
   */
  private analyzeBasicStats(channelData: Float32Array[], sampleRate: number): BasicAudioStats {
    ...
    const numBlocks512 = Math.floor(length / hop512);
    const numBlocks100ms = Math.floor(length / hop100ms);
    ...
    // Running block stats
    let currentBlockSumSq512 = 0;
    let currentBlockSumSq100ms = 0;
    let currentBlockPeak100ms = 0;
    ...
    for (let i = 0; i < length; i++) {
      ...
      currentBlockSumSq512 += sMonoSq;
      currentBlockSumSq100ms += sMonoSq;
      const absMono = Math.abs(sMono);
      if (absMono > currentBlockPeak100ms) currentBlockPeak100ms = absMono;

      // Commit block stats
      if ((i + 1) % hop512 === 0) {
        const b = (i + 1) / hop512 - 1;
        if (b < numBlocks512) {
          blockEnergy512[b] = Math.sqrt(currentBlockSumSq512 / hop512);
          currentBlockSumSq512 = 0;
        }
      }

      if ((i + 1) % hop100ms === 0) {
        const b = (i + 1) / hop100ms - 1;
        if (b < numBlocks100ms) {
          blockEnergy100ms[b] = currentBlockSumSq100ms;
          blockPeaks100ms[b] = currentBlockPeak100ms;
          currentBlockSumSq100ms = 0;
          currentBlockPeak100ms = 0;
        }
      }
  ...
  private async analyzeTemporalFeatures(
    ...
    // ⚡ Bolt: Use pre-calculated block energy for envelope
    const envelope = stats.blockEnergy512;
    const hopSize = stats.hop512;
   ...
   * ⚡ Bolt: Uses pre-calculated 100ms blocks to aggregate section energy.
   */
  private detectSections(audioBuffer: AudioBuffer, stats: BasicAudioStats): AudioSection[] {
    ...
    const blocksPerSection = Math.floor(sectionLengthSeconds / 0.1); // 0.1s blocks
    const numBlocks = stats.blockEnergy100ms.length;

    for (let b = 0; b < numBlocks; b += blocksPerSection) {
      let totalSumSq = 0;
      const actualBlocks = Math.min(blocksPerSection, numBlocks - b);

      for (let i = 0; i < actualBlocks; i++) {
        totalSumSq += stats.blockEnergy100ms[b + i];
      }

      const totalSamples = actualBlocks * stats.hop100ms;
      const energy = Math.sqrt(totalSumSq / (totalSamples + 1e-10));

      const startTime = (b * stats.hop100ms) / sampleRate;
      const endTime = Math.min(startTime + sectionLengthSeconds, duration);
   ...
   * ⚡ Bolt Optimization: Uses pre-calculated block metrics (O(N/hop)).
   */
  private calculateLoudnessOverTime(stats: BasicAudioStats, sampleRate: number): LoudnessPoint[] {
    const { blockEnergy100ms: blockEnergy, blockPeaks100ms: blockPeaks, hop100ms: hopSize } = stats;
   ...
   * ⚡ Bolt: Uses pre-calculated 100ms blocks (O(N/hop)).
   */
  private detectSilence(stats: BasicAudioStats, sampleRate: number): any[] {
    ...
    if (inSilence) {
      const duration = (blockEnergy.length * hopSize) / sampleRate - silenceStart;
      if (duration >= minDuration) {
        silentSections.push({
          startTime: silenceStart,
          endTime: (blockEnergy.length * hopSize) / sampleRate,
          duration,
          threshold,
        });
      }
