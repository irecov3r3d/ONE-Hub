 ...
 * 4. Integrated Spectral Synergy: Leverages shared 8192-point FFT magnitudes to detect musical key
 *    and pitch classes simultaneously, eliminating redundant O(M) accumulation passes.
 */
export class AudioAnalysisService {
   ...
   * ⚡ Bolt Optimization: representative sampling of 200 frames.
   */
  private calculateSpectralFlux(
    ...
    const numFrames = Math.floor((samples.length - fftSize - hopSize) / hopSize);
    const targetFrames = 200;
    const step = Math.max(1, Math.floor(numFrames / targetFrames));

    for (let f = 0; f < numFrames; f += step) {
      const i = f * hopSize;
      if (i >= samples.length - fftSize - hopSize) break;
      let flux = 0;
   ...
   * ⚡ Bolt Optimization: Integrated Spectral Synergy with key detection using shared 8192 magnitudes.
   */
  ...
  /**
   * Analyze pitch classes from pre-calculated normalized chromagram.
   * ⚡ Bolt: Bypasses redundant passes by directly converting chromagram weights.
   */
