...
export class AudioAnalysisService {
   ...
   * ⚡ Bolt Optimization: Reuses pre-calculated magnitudes for key detection.
   */
  private async analyzeMusicalFeatures(
    ...
    const keyData = await this.detectKey(stats.mono, audioBuffer.sampleRate, magnitudes8192);
  ...
  /**
   * Detect musical key.
   * ⚡ Bolt Optimization: Uses real Krumhansl-Schmuckler key detection and reuses FFT magnitudes.
   */
  private async detectKey(
    ...
    // ⚡ Bolt: Use pre-calculated magnitudes if available to bypass redundant FFT
    const result = magnitudes
      ? await this.keyDetector.detectKeyFromMagnitudes(magnitudes, sampleRate)
      : await this.keyDetector.detectKey({ sampleRate, getChannelData: () => samples, length: samples.length } as any);
