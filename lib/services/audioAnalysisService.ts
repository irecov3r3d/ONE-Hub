...
export class AudioAnalysisService {
  ...
  private async analyzeMusicalFeatures(
    ...
    const keyData = this.detectKey(magnitudes8192, audioBuffer.sampleRate);
  ...
  /**
   * Detect musical key using optimized AdvancedKeyDetection.
   * ⚡ Bolt Optimization: Uses pre-calculated linear magnitudes to avoid redundant FFT.
   */
  private detectKey(magnitudes: Float32Array, sampleRate: number): {
    key: string;
    scale: string;
    confidence: number;
  } {
    const result = this.keyDetector.detectKeyFromMagnitudes(magnitudes, sampleRate);
    return {
      key: result.key,
      scale: result.scale,
