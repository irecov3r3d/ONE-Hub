...
export class AudioAnalysisService {
   ...
   * Also pre-calculates energy and peaks for 512-sample and 100ms blocks.
   */
  private analyzeBasicStats(channelData: Float32Array[], sampleRate: number): BasicAudioStats {
    ...
    let currentSumSq512 = 0;
    let currentPeak512 = 0;
    let currentSumSq100ms = 0;
    ...
    for (let i = 0; i < length; i++) {
      ...
      // Block-level accumulation (using mono for these metrics)
      const absMono = Math.abs(sMono);
      const sSqMono = sMono * sMono;

      // 512-sample blocks
      currentSumSq512 += sSqMono;
      if (absMono > currentPeak512) currentPeak512 = absMono;

      if ((i + 1) % hop512 === 0) {
        const blockIdx = (i + 1) / hop512 - 1;
        if (blockIdx < numBlocks512) {
          blockEnergy512[blockIdx] = currentSumSq512;
          blockPeaks512[blockIdx] = currentPeak512;
        }
        currentSumSq512 = 0;
        currentPeak512 = 0;
      }
      ...
      // 100ms blocks
      currentSumSq100ms += sSqMono;
      if (absMono > currentPeak100ms) currentPeak100ms = absMono;

      if ((i + 1) % hop100ms === 0) {
        const blockIdx = (i + 1) / hop100ms - 1;
        if (blockIdx < numBlocks100ms) {
          blockEnergy100ms[blockIdx] = currentSumSq100ms;
  ...
  private async analyzeTemporalFeatures(
    ...
    // ⚡ Bolt Optimization: Use pre-calculated 512-sample block energy (O(N/hop))
    // instead of redundant O(N) buffer traversal.
    const hopSize = 512;
    const envelope = stats.blockEnergy512.map(energy => Math.sqrt(energy / hopSize));
   ...
   * ⚡ Bolt Optimization: Uses pre-calculated 100ms blocks to reduce O(N) traversal to O(N/hop).
   */
  private detectSections(audioBuffer: AudioBuffer, stats: BasicAudioStats): AudioSection[] {
    const duration = audioBuffer.duration;
    const sectionLength = 8;
    const hop100ms = Math.floor(audioBuffer.sampleRate * 0.1);
    const blocksPerSection = Math.floor((sectionLength * audioBuffer.sampleRate) / hop100ms);
    const sections: AudioSection[] = [];

    for (let time = 0; time < duration; time += sectionLength) {
      const endTime = Math.min(time + sectionLength, duration);
      const startBlock = Math.floor((time * audioBuffer.sampleRate) / hop100ms);
      const endBlock = Math.floor((endTime * audioBuffer.sampleRate) / hop100ms);
      ...
      const totalSamples = blockCount * hop100ms;
      const energy = Math.sqrt(totalEnergy / (totalSamples + 1e-10));
   ...
   * ⚡ Bolt Optimization: Uses pre-calculated 100ms blocks to reduce O(N) traversal to O(N/hop).
   * 1. Aggregate 4 pre-calculated blocks to compute metrics for the sliding window (400ms).
   */
  ...
  private detectSilence(stats: BasicAudioStats, sampleRate: number): any[] {
    ...
    for (let b = 0; b < stats.blockEnergy100ms.length; b++) {
      const rms = Math.sqrt(stats.blockEnergy100ms[b] / (hopSize + 1e-10));
    ...
    if (inSilence) {
      const endTime = stats.mono.length / sampleRate;
