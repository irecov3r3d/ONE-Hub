...
export class AudioAnalysisService {
   ...
   * ⚡ Bolt Optimization: Reuses shared magnitudes for key and pitch detection.
   */
  private async analyzeMusicalFeatures(
    ...
    const keyData = this.detectKeyFromMagnitudes(magnitudes8192, audioBuffer.sampleRate);
  ...
  /**
   * ⚡ Bolt Optimization: Detect musical key using pre-calculated magnitudes.
   */
  private detectKeyFromMagnitudes(magnitudes: Float32Array, sampleRate: number): {
    key: string;
    scale: string;
    confidence: number;
    chromagram: number[];
  } {
    return this.keyDetector.detectKeyFromMagnitudes(magnitudes, sampleRate);
  }

  /**
   * ⚡ Bolt Optimization: Analyze pitch class content from pre-calculated chromagram.
   */
  private analyzePitchClassesFromChroma(chromagram: number[]): PitchClass[] {
    const notes = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'];
    const pitchClasses: PitchClass[] = [];

    // Normalize chromagram for strength values
