...
export class AdvancedKeyDetection {
  ...
  // ⚡ Bolt Optimization: Cache pitch class mappings to avoid Math.log2 in hot loop
   ...
   * ⚡ Bolt Optimization: Supports polymorphic input for zero-copy segment analysis
   */
  async detectKey(
   ...
   * 2. Uses cached pitch-class mapping to eliminate Math.log2 and range checks in hot loop.
   */
  private async calculateChromagram(
    ...
    // ⚡ Bolt: Get or create pitch class map for this FFT size/sample rate
    ...
    for (let i = 0; i < spectrum.length; i++) {
      ...
      // ⚡ Bolt: pitchMap contains -1 for bins outside 80Hz-5000Hz range
      if (pitchClass !== -1) {
  ...
  /**
   * ⚡ Bolt Optimization: Pre-calculates pitch class mapping for an entire FFT spectrum.
   * Eliminates millions of Math.log2 calls and range checks during batch processing.
   */
  private getPitchClassMap(fftSize: number, sampleRate: number): Int8Array {
    ...
    if (!map) {
      ...
      for (let i = 0; i < numBins; i++) {
        ...
        // Filter range: 80Hz - 5000Hz (musical fundamentals)
   ...
   * Uses Float32Array.subarray() to avoid O(S * C * N) allocations of OfflineAudioContext and AudioBuffers.
   */
  async detectChordProgression(
    ...
    for (let i = 0; i < segments; i++) {
      ...
      // ⚡ Bolt: Use .subarray() for zero-copy view of the audio data
      const segment = channelData.subarray(
        startSample,
        Math.min(startSample + segmentLength, channelData.length)
      );

      // Detect key/chord for this segment using optimized polymorphic detectKey
