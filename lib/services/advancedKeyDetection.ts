...
export class AdvancedKeyDetection {
   ...
   * ⚡ Bolt Optimization: Supports raw sample input to avoid redundant AudioBuffer allocations.
   */
  async detectKey(
   ...
   * ⚡ Bolt Optimization: Uses pre-calculated linear magnitudes and supports polymorphic input.
   */
  private async calculateChromagram(
    ...
    const { spectrum, linearMagnitudes } = await this.fftEngine.performFFT(input, 8192, sampleRate);
    const sr = input instanceof AudioBuffer ? input.sampleRate : (sampleRate || 44100);
  ...
  async detectChordProgression(
    ...
    for (let i = 0; i < segments; i++) {
      ...
      // ⚡ Bolt Optimization: Use zero-copy subarray instead of extractSegment
      ...
      const keyData = await this.detectKey(segmentSamples, sr);
