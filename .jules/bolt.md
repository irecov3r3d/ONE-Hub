...
## 2026-05-30 - Pitch Class Cache and Zero-Copy Subarray Chord Progression
**Learning:** In audio key detection pipelines, performing heavy mathematical mapping (such as converting frequencies to MIDI notes and pitch classes using `Math.log2` and rounding) inside hot FFT loops for multiple overlapping segments or segments of audio leads to significant CPU churn. Offloading this mapping into a static `pitchClassCache` (`Map<string, Int8Array>`) keyed by `fftSize` and `sampleRate` yields O(1) pitch class index lookups and bypasses math functions and conditional boundary checks entirely. Additionally, extracting segments using `Float32Array.subarray()` avoids the substantial overhead of instantiating `OfflineAudioContext` and new `AudioBuffer` objects per segment, creating a completely zero-copy extraction path.
**Action:** Use typed-array lookups for frequency-bin classifications in hot loops, and prefer direct `Float32Array.subarray()` over `OfflineAudioContext` for segmenting large audio streams.
