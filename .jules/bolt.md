...
## 2024-05-22 - Mathematical Identities for Loop Optimization
**Learning:** Deriving global Mid/Side energy using mathematical identities ($\sum mid^2 = 0.25 * (\sum L^2 + \sum R^2 + 2\sum LR)$ and $\sum side^2 = 0.25 * (\sum L^2 + \sum R^2 - 2\sum LR)$) outside the primary $O(N)$ loop is significantly faster than performing per-sample additions/subtractions. Combining this with loop unswitching for mono/stereo paths reduces CPU cycles and branch prediction overhead in high-frequency audio buffers.
**Action:** Always check if global metrics can be derived from accumulated primitive stats (Sum, SumSq, Correlation) to keep the inner loop as lean as possible.
