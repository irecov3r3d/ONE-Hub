...
## 2026-05-07 - Cached Mapping and Hoisted Templates in Key Detection
**Learning:** Chromagram-based key detection loops (converting frequency to MIDI/pitch-class) in high-frequency analyses (e.g., chord progression over rolling windows) perform redundant calculations and object allocations. Because the mapping of FFT bins to pitch classes depends solely on `fftSize` and `sampleRate`, we can cache this entire mapping in a static `Int8Array` on module initialization, entirely bypassing O(N) log2 math and boundary checks. Additionally, hoisting rotated Krumhansl-Schmuckler major/minor templates to module level eliminates $O(C \times 12)$ array slices and garbage collection pressure in Pearson correlation passes.
**Action:** For iterative chromagram and template matching routines, pre-calculate bin-to-pitch class maps and rotate reference templates on startup to achieve zero-allocation hot paths.
