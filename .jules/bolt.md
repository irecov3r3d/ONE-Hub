...
## 2026-05-06 - Zero-Copy Segment Analysis and Pitch Class Caching
**Learning:** For segment-based analysis (like chord detection), creating `AudioBuffer` copies via `OfflineAudioContext` is extremely expensive due to allocations and data copying. Switching to `Float32Array.subarray()` and supporting direct `Float32Array` in the FFT engine provides a massive performance boost. Furthermore, caching the mapping of FFT bins to pitch classes in a static `Map` avoids millions of `Math.log2` calls in the chromagram accumulation loop.
**Action:** For time-sliced audio analysis, use zero-copy `TypedArray.subarray()` and cache frequency-to-musical-note mappings whenever possible to minimize GC pressure and CPU cycles.
