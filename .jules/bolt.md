...
## 2026-05-29 - Zero-Copy Polymorphic Segment Analysis
**Learning:** Extracting temporary audio segments as dedicated `AudioBuffer` objects via `OfflineAudioContext` for frame-by-frame analysis creates massive browser thread and GC overhead. Supporting polymorphic types (`AudioBuffer | Float32Array`) in core signal-processing pipelines allows the analysis engines to consume raw `Float32Array.subarray()` views directly, resulting in a 2.07x speedup and zero allocation overhead.
**Action:** Always expose a polymorphic or raw TypedArray path in analysis engines (`FastFFTEngine`) to enable zero-copy segment-based slice processing.
