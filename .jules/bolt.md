...
## 2026-05-07 - Precomputed Ending Cache in Rhyme Analysis
**Learning:** Evaluating rhyme schemes in lyrics with O(N^2) loops where each step performs regex, trim, split, and string slicing creates significant CPU overhead and garbage collection pressure due to high allocation of transient strings. By precalculating and caching the two-character endings of each line's last word in a single O(N) pass, we can reduce nested matching to cheap string identity checks. Additionally, baseline algorithms can contain quirks like single-replacement regex behavior (e.g. `replace(/[.,!?;:]/, '')` without global flag) which must be replicated to maintain bug compatibility.
**Action:** Always precompute and cache properties of elements prior to nested loop comparison, and pay close attention to regex global search/replace flags when trying to achieve 100% bug compatibility with baseline code.
