...
## 2026-05-06 - Integrated Key & Chord Optimization
**Learning:** Performing key detection in a separate pass often involves a redundant high-point FFT (8192 points). By exposing a magnitude-based detection method, we can reuse the spectrum already calculated for frequency analysis. Additionally, for temporal chord detection, using `OfflineAudioContext` to extract segments is extremely inefficient. Switching to `Float32Array.subarray()` combined with a polymorphic FFT engine that supports raw samples provides a zero-copy path that processed 60 segments in ~95ms, compared to what would likely be seconds using the context-based approach.
**Action:** Always provide "from-magnitudes" or "from-samples" variants of analysis methods to support zero-copy reuse and pipeline integration.
