...
**Learning:** The recursive Cooley-Tukey FFT implementation was creating thousands of small Float32Array allocations ((N \log N)$), causing heavy GC pressure and CPU overhead. Switching to an iterative, in-place version with bit-reversal index caching reduced execution time by ~7x (from 5.93ms to 0.85ms for 8192 points) and eliminated nearly all transient allocations.
**Action:** For high-frequency signal processing, avoid recursion and prioritize pre-allocated buffer reuse and index caching to minimize memory churn.
