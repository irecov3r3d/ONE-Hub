...
## 2026-05-15 - One-Pass Cached Rhyme Scheme Analysis
**Learning:** Performing rhyme scheme analysis on raw song lyrics on every keystroke in components like `LyricEditor` triggers extreme performance lag and thread blocking on larger text files. This is because the algorithm splits the lines and repeatedly extracts the last word of each line using a regex-based helper inside a nested loop, resulting in O(N^2) string splits, trims, lowercasing, and regex evaluations. By pre-calculating the last word of each line in a single O(N) pass, the nested loop can perform instantaneous O(1) checks, reducing the overall execution time by ~4.57x and completely eliminating garbage collection stutter.
**Action:** When working with nested loops that analyze text segments (like words, syllables, or rhymes), lift all string parsing, regex, and formatting transformations out of the loop into a single pre-calculated array.
