...
## 2026-05-07 - Pre-Calculated String Normalization in Nested Loops
**Learning:** Performing regex replaces, substring manipulation, casing transformations, and splits (such as `getLastWord`) repeatedly inside nested loops (like rhyme scheme analysis) creates an O(N^2) complexity bottleneck. Pre-calculating these static word values into a flat array reduces the execution complexity to O(N), saving CPU cycles and garbage-collection overhead.
**Action:** Always pre-calculate string manipulations or normalization functions outside of nested loops to maintain linear algorithmic complexity.
